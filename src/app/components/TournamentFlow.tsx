...
interface TournamentFlowProps {
  rounds: Round[];
  handleSelectWinner: (roundIndex: number, matchId: string, winner: Player) => void;
            ...
            {round.matchups.map((match) => (
              <S.MatchupContainer key={match.id}>
                ...
                <S.PlayerName
                  ...
                  onClick={() => handleSelectWinner(roundIndex, match.id, match.player2)}
                >
                  {match.player2.name}
                </S.PlayerName>
                {match.winner && (
