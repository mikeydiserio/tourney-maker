  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < nebulaParticleCount; j++) {
        ...
        const variation = 0.3;
        nebulaColorsArray[j3] = Math.min(
          1,
          color.r + (Math.random() - 0.5) * variation
        );
        nebulaColorsArray[j3 + 1] = Math.min(
          1,
          color.g + (Math.random() - 0.5) * variation
        );
        nebulaColorsArray[j3 + 2] = Math.min(
          1,
      ...
      nebulaGeometry.setAttribute(
        "color",
        new THREE.BufferAttribute(nebulaColorsArray, 3)
      );
    ...
    const animate = () => {
      ...
      const starOpacityAttribute = starGeometry.getAttribute(
        "opacity"
